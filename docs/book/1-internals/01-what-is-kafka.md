...
proof:
  mode: delegated
  status: 위임
  method: "1장 개념(offset 이동·순서·pull)의 런타임 증명은 II권 Spring Step에 위임(1.6)"
  pending: []
  done: []
...
```mermaid
graph LR
    P["Producer<br/>(쓴다)"] -->|append| B["Broker<br/>(저장한다 · 파티션)"]
    B -->|"읽는다 (pull)"| C1["Consumer A"]
    B -->|"읽는다 (pull)"| C2["Consumer B"]
    C1 -->|"offset 커밋"| O["__consumer_offsets<br/>어디까지 읽었나"]
    C2 -->|"offset 커밋"| O
...
> 여러 Broker가 모여 이루는 **Cluster**, 그 두뇌인 **Controller**, Consumer Group을 관리하는 **Coordinator**, 그리고 offset이 사는 `__consumer_offsets`는
