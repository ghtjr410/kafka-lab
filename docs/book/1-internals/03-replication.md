...
- **리더 자신도 항상 ISR의 일원이다.** ISR은 "리더 + 따라잡은 팔로워들"이지 "리더를 뺀 팔로워들"이 아니다. 리더는 자기 로그가 기준이라 늘 in-sync이기 때문이다. 그래서 `min.insync.replicas=2`는 "리더 1 + 팔로워 1"로도 충족된다(→ 3.5).
- **ISR은 고정 명단이 아니라 RF 복제본의 *동적 부분집합*이다.** 따라잡으면 들고 뒤처지면 빠진다. 이때 빠진 나머지가 **OSR(Out-of-Sync Replicas)** 다. 따로 떼어 둔 복제본이 아니라 *원래 RF 멤버인데 지금 뒤처진 것*이다.
...
- 팔로워가 `replica.lag.time.max.ms` 안에 따라오지 못하면 **ISR에서 축출**(= OSR로)된다. 다시 따라잡으면 복귀한다.
...
리더 브로커가 죽으면 컨트롤러가 **ISR 안에서 새 리더를 뽑는다**(→ 4장). ISR 안에서 뽑으니 새 리더는 HW까지의 데이터를 갖고 있다 → **무손실**. 문제는 그 ISR마저 *전부* 죽었을 때다. 한 파티션이 정상 선출에서 unclean 갈림길까지 어떻게 내려가는지 따라가 보자.
