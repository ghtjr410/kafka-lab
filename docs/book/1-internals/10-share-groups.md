...
이 책의 [1장](./01-what-is-kafka.md)은 "Kafka는 큐가 아니라 로그"라고 했고, [5장](./05-coordination.md)은 "한 파티션은 한 consumer에게만"이라고 했다. **Share Group(KIP-932)** 은 이 두 전제에 단서를 단다. Kafka 4.2에서 GA된 이 기능으로, Kafka는 **로그 위에 큐 시맨틱을 얹는다**. (저장은 여전히 append-only 로그이고, ack·락 상태만 `__share_group_state`에 별도 추적된다. KIP-932는 "Kafka에 큐를 추가"한 게 아니라 토픽으로 큐 유스케이스를 수용하는 share group을 도입한 것이다.) `[KIP-932 · docs @4.2]`
...
그런데 consumer group은 정반대 전제 위에 있다. **배타 배정**(5.1: 한 파티션=한 consumer, 그래서 consumer ≤ 파티션)과 **로그 retention**([2장](./02-log-abstraction.md): 읽어도 안 사라지고 offset만 전진). 이 둘은 큐의 요구와 정면으로 부딪힌다:
...
| | Consumer Group ([5장](./05-coordination.md)) | Share Group (이 장) |
|---|---------------------|---------------------|
...
레코드별 상태는 어딘가 영속돼야 한다. 그 주체가 **Share Coordinator**다. [5장](./05-coordination.md) Group·[7장](./07-transactions.md) Transaction Coordinator에 이은 **셋째 coordinator**다. ([4장](./04-consensus.md) **Controller**는 합의·메타데이터를 맡는 별개 역할이라 coordinator와 결이 다르다. 아래 표는 비교용이다.)

| coordinator | 관리 대상 | 저장 토픽 | 장 |
|-------------|----------|----------|-----|
| Controller | 클러스터 메타데이터 | `__cluster_metadata` | [4장](./04-consensus.md) |
| Group Coordinator | consumer group offset | `__consumer_offsets` | [5장](./05-coordination.md) |
| Transaction Coordinator | 트랜잭션 상태 | `__transaction_state` | [7장](./07-transactions.md) |
...
→ 다시 [2장](./02-log-abstraction.md)의 "메타데이터도 로그": share 소비 상태조차 로그에 쌓인다.
...
두 RPC는 `(GroupId, MemberId)`로 식별되는 **share session**으로 문맥을 유지한다. → [9장](./09-client-runtime.md)의 fetch가 "offset부터 순서대로 당기기"라면, ShareFetch는 "available한 레코드를 락 걸어 가져오기"다.
...
- **Exactly-Once 미지원**: 시맨틱은 **at-least-once**다([7장](./07-transactions.md)의 트랜잭션 EOS는 consumer group/produce 경로 한정). 단 토픽에 트랜잭션 레코드가 있으면 `share.isolation.level`로 read_committed 여부는 고를 수 있다.
...
- 연결: 1.2(로그 위에 큐 시맨틱) · 5.1(배타 배정의 경계) · [2장](./02-log-abstraction.md)(메타데이터도 로그) · 9.7(fetch 대비)
