...
>
> **📏 작성 규칙 (중요):**
> - README의 각 불릿은 **"한 주제를 한 문장으로"** 표현한다 — 여기는 *지도(index)*이지 본문이 아니다.
> - 상세 서술·다이어그램·코드·증명은 개별 `NN-*.md`(산문화)에만 둔다. **README에 본문을 쏟지 말 것.**
> - 워크플로우: README 불릿로 무엇을 적을지 정함 → 확정 장만 `NN-*.md`로 옮겨 산문화 → 산문에서 새 포인트가 생기면 **README 불릿(-D)에 한 문장으로 반영**(양방향 동기화).
> - **🔒 불변식: 다른 장·권은 named link로만 가리킨다. 본문에 장 번호·권 번호를 *숫자/텍스트로 박지 말 것*.** (`(I권 7장)`·`II권 Consumer` ❌ → `[저장 엔진](08-storage-engine.md)`·`[II권](../2-spring/README.md)` ✅) — 순서가 바뀌면 링크는 *깨져 보이지만* 박힌 숫자는 *조용히 stale*된다. LSO·권번호 드리프트가 전부 이 위반에서 났다.
