...
한 poll 배치 위에 네 모드의 커밋 *지점*을 겹쳐 보면:

```mermaid
sequenceDiagram
    participant L as Listener
    participant K as Broker (committed offset)
    Note over L,K: poll() → r0, r1, r2 (한 배치)
    L->>L: r0 처리
    L-->>K: RECORD → 매 건 commit · MANUAL_IMMEDIATE → ack() 즉시 commit
    L->>L: r1 처리
    L-->>K: RECORD → commit
    L->>L: r2 처리
    L-->>K: RECORD → commit
    Note over L,K: ── 다음 poll 경계 ──
    L->>K: BATCH(기본) · MANUAL → 여기서 일괄 commit
```
