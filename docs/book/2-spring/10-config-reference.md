...
prose: done
proof:
  tests: [s01_producer, s02_consumer, s03_partition, s04_rebalancing, s05_dlq, s06_eos]
  gaps: []
upstream: ["../1-internals/03-replication.md", "../1-internals/05-coordination.md",
           "../1-internals/06-ordering-atomicity.md", "../1-internals/07-transactions.md",
...
| 설정 | 기본값 | 검증 | 다루는 곳 |
|------|--------|------|----------|
| `acks` | `all` (3.0+ idempotence=true가 강제) | ✓ | [본편](../../../src/test/java/com/example/kafka/s01_producer/README.md) · [설정 조합의 함정](./08-config-combination-traps.md) |
| `enable.idempotence` | `true` (3.0+) | ✓ `[KIP-679]` | [설정 조합의 함정](./08-config-combination-traps.md) · [I권 멱등](../1-internals/06-ordering-atomicity.md) |
| `max.in.flight.requests.per.connection` | 5 | ✓ | [설정 조합의 함정](./08-config-combination-traps.md) |
| `linger.ms` | 0 | ✓ `[code @3.7]` | [본편](../../../src/test/java/com/example/kafka/s01_producer/README.md) |
| `batch.size` | 16384 | ✓ `[code @3.7]` | [본편](../../../src/test/java/com/example/kafka/s01_producer/README.md) |
| `buffer.memory` | 33554432 (32MB) | ✓ `[code @3.7]` | [본편](../../../src/test/java/com/example/kafka/s01_producer/README.md) · [I권 클라이언트 런타임](../1-internals/09-client-runtime.md) |
...
**[증명]** [s01 producer](../../../src/test/java/com/example/kafka/s01_producer/README.md) · [s06 EOS](../../../src/test/java/com/example/kafka/s06_eos/README.md)
...
| 설정 | 기본값 | 검증 | 다루는 곳 |
|------|--------|------|----------|
| `enable.auto.commit` | (Spring이 `false`로 강제) | ✓ | [본편](../../../src/test/java/com/example/kafka/s02_consumer/README.md) |
| `auto.offset.reset` | `latest` | ✓ `[code @3.7]` | [본편](../../../src/test/java/com/example/kafka/s02_consumer/README.md) |
| `isolation.level` | `read_uncommitted` | ✓ | [본편](../../../src/test/java/com/example/kafka/s06_eos/README.md) · [설정 조합의 함정](./08-config-combination-traps.md) · [I권 트랜잭션](../1-internals/07-transactions.md) |
| `heartbeat.interval.ms` | 3000 | ✓ `[code @3.7]` | [설정 조합의 함정](./08-config-combination-traps.md) · [I권 조정](../1-internals/05-coordination.md) |
| `session.timeout.ms` | 45000 | ✓ `[code @3.7]` | [설정 조합의 함정](./08-config-combination-traps.md) · [I권 조정](../1-internals/05-coordination.md) |
| `max.poll.interval.ms` | 300000 | ✓ `[code @3.7]` | [설정 조합의 함정](./08-config-combination-traps.md) · [코드 구조·순서의 함정](./09-code-order-traps.md) |
...
## 10.4 Listener (Spring `ContainerProperties`)

| 설정 | 의미 | 다루는 곳 |
|------|------|----------|
| `AckMode` | 커밋 시점(BATCH·RECORD·MANUAL·MANUAL_IMMEDIATE) | [본편](../../../src/test/java/com/example/kafka/s02_consumer/README.md) · [commit 위치](./09-code-order-traps.md) |
| `concurrency` | 컨테이너 스레드 수 (`concurrency × 인스턴스 ≤ 파티션`) | [본편](../../../src/test/java/com/example/kafka/s03_partition/README.md) |
| `CommonErrorHandler` / `DefaultErrorHandler` | retry(BackOff) + DLQ(recoverer) | [본편](../../../src/test/java/com/example/kafka/s05_dlq/README.md) · [코드 구조·순서의 함정](./09-code-order-traps.md) |
| `@RetryableTopic` | non-blocking retry(retry 토픽) | [코드 구조·순서의 함정](./09-code-order-traps.md) |

**[증명]** [s02 consumer](../../../src/test/java/com/example/kafka/s02_consumer/README.md) · [s03 partition](../../../src/test/java/com/example/kafka/s03_partition/README.md) · [s05 DLQ](../../../src/test/java/com/example/kafka/s05_dlq/README.md)
...
| 증상 | 가장 흔한 원인 → 어디 |
|------|----------------------|
| 메시지가 **유실**됐다 | 처리 전 커밋 / 예외 삼킴 → [Consumer & Offset](./02-consumer-offset.md)·[9.2](./09-code-order-traps.md) · skip(DLQ 아님) → [DLQ & 에러 처리](./05-error-handling-dlq.md) · `send()` async 실패 무시 → [Producer 보장](./01-producer.md) |
...
| 롤링 배포 때 **멈춘다** | 리밸런싱 / 타이밍 → [리밸런싱 & 배포](./04-rebalancing.md)·[8.3](./08-config-combination-traps.md) |
| 역직렬화에서 **죽는다**(무한 재폴링) | `ErrorHandlingDeserializer` 부재(poison-pill) → [직렬화 & 스키마](./07-serialization.md)·[9.1](./09-code-order-traps.md) |
| 멱등이 **조용히** 꺼졌다 | acks / max.in.flight 충돌 → [8.1](./08-config-combination-traps.md) |
| 설정이 **안 먹는다** | `properties.*` 맵 우회 / `spring.jackson` 미적용 → 위 *설정 주입* · [직렬화 & 스키마](./07-serialization.md) |
| 컨슈머가 **퇴출**된다 | 리스너 blocking / poll 초과 → [9.3](./09-code-order-traps.md)·[8.3](./08-config-combination-traps.md) |
| 새 그룹인데 **메시지가 안 온다** | `auto.offset.reset=latest`(기본) → [Consumer & Offset](./02-consumer-offset.md) |
