...
>
> 🧭 **이 표는 "무엇이 기본값인가"의 인덱스다. "언제 어느 값으로 돌리는 게 유리한가(trade-off)"는** → [III권 10장 — 설정 의사결정 트리(CAP·PACELC)](../3-operations/10-config-decision-tree.md). 특히 `acks` · `linger.ms` · `max.block.ms`.
