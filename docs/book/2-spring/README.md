...
>
> 📐 집필 공통 규칙(README=인덱스 · SSOT 표 · 상태 2축 · 증명 모델 등)은 [전체 표지](../README.md)가 단일 진실이다 — 여기선 가장 자주 위반되는 불변식 하나(아래 🔒)만 재게시한다.
...
> **증명 모델** — II권의 증명은 **`KafkaTestBase`를 상속한 통합 테스트**(실제 docker 브로커 `localhost:9092` — `@EmbeddedKafka` 아님, I권과 브로커 공유)다. Kafka *동작 자체*(acks·멱등·파티션·offset)는 **raw kafka-clients**로, **Spring 특유 동작**(`AckMode`·`DefaultErrorHandler`·DLQ)은 `KafkaTemplate`/`@KafkaListener`로 검증한다 — 후자(s01·s02·s05)가 II권을 별도 권으로 만드는 부분이다.
> - **본편(1~7장)**: 각 장의 증명은 기존 **Step 테스트**(`src/test/.../sNN_*`, `KafkaTestBase` 상속)가 담당한다.
