...
각 권 README는 아래 규칙을 따른다. **권 README에 재서술하지 말고 이곳을 링크**한다(불변식만 자주 위반되므로 각 권에 한 줄 재게시 허용). 1권에서 LSO·권번호·증명모델 드리프트가 *세 번* 났고, 전부 "같은 규칙을 여러 곳에 적어 어긋난" 것이라 여기 단일화한다.

1. **README = 인덱스(지도).** 각 불릿은 **"한 주제를 한 문장으로"**. 상세·다이어그램·코드·증명은 **개별 `NN-*.md`(산문)** 에만. 산문에서 새 포인트가 생기면 README 불릿(-D)으로 **역반영**(양방향 동기화).

2. **🔒 불변식 — 다른 장·권은 named link로만.** 장 번호·권 번호를 본문에 *숫자/텍스트로 박지 말 것.* (`(I권 7장)`·`II권 Consumer` ❌ → `[저장 엔진](1-internals/08-storage-engine.md)`·`[II권](2-spring/README.md)` ✅) 순서가 바뀌면 링크는 *깨져 보이지만* 박힌 숫자는 *조용히 stale*된다.

3. **📋 SSOT 표 — 교차 요소의 "정의 위치"는 표 1곳.** 같은 개념이 여러 장에 걸치면 정의 위치를 README의 "교차 요소" 표에 한 번만 적고 본문은 따른다. 본문 괄호에 `(N장 정의)`로 재서술하면 거의 반드시 드리프트한다(표·산문이 어긋나면 *산문 기준으로 표를 고친다*).

4. **🚦 상태 마커 2축.** `[산문]` ✅완료 / 📝아웃라인 / 🚧작업중 — 그리고 `[executable 증명]`은 **별도 축**. 산문 ✅가 증명 완료를 뜻하지 않는다.
