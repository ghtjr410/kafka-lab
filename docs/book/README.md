...
7. **깊이는 in-store 층위로: `<details>` 접기.** 이 lab은 지식저장소다. 독자(=저자)가 막히는 지점이 곧 지식 구멍이고, 그 구멍은 **밖이 아니라 책 안에, 한국어로** 메운다. 그래서 깊이를 외부 링크로 떠넘기지 말고 `<details>`로 *그 자리에* 접어 둔다. 표면(결론·핵심)은 본문에 항상 보이고, 한 층 아래 *왜*·CS 바닥은 펼쳐야 보인다(평소 접힘=표면 가벼움, 펼침=점프 없음, 탑다운으로 바닥까지).

   - **깊이 판단(depth test).** "여기서 막히나?" → 막히면 그게 구멍 → 그 자리 `<details>`로 채운다. *외부 완전성*이 아니라 *막힘*이 깊이를 정한다. 안 막히면 표면으로 끝.
   - **얼마나 깊이(바닥선).** top-down으로 내려가되 *더 내려가도 그 장의 '왜'가 더 또렷해지지 않는 지점*에서 멈추고 외부(DDIA 등)로 넘긴다(예: 디스크 "왜 빠른가"는 헤드·트랙·섹터까지가 바닥, 그 아래 FTL·NAND는 외부). 깊은 칸이 길면 **중첩 `<details>`로 계단**처럼, 원하는 만큼만 더 펼치게.
   - **자리 분담.** 용어의 정확한 정의 → GLOSSARY(SSOT, 본문은 앵커 링크) / 한 층 아래 *왜* → `<details>` / 절대 바닥 풀세트 → DDIA(한국어판) 등 외부 참조. **KIP·소스 라벨은 집필·신뢰용 출처 도장이지 독자용 "더 읽을 곳"이 아니다**(한국 독자는 영어 KIP를 읽지 않는다).
   - **접힌 내용도 검증한다.** 틀린 딥다이브가 접혀 있으면 더 위험(안 들킴). `<details>` 안의 사실도 아래 "사실 검증"을 똑같이 받는다.
   - 형식: `<summary>`는 "왜? 한 줄" 또는 "심화: 주제". GitHub에서 렌더되고 mermaid는 펼칠 때 그려진다.
